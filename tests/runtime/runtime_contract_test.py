...
class RuntimeGateContractTest(unittest.TestCase):
    ...
    def test_runtime_versions_and_fault_scenarios_are_pinned(self) -> None:
        ...
        self.assertIn("+quit || true", dockerfile)
        self.assertIn("test -r /opt/hlds/engine_i486.so", dockerfile)
        self.assertIn("test -r /opt/hlds/filesystem_stdio.so", dockerfile)
        ...
        self.assertIn("redis_runtime_restore_defaults", runner)
